...
window.BLOWTORCH_LIVE = (() => {
  ...
  function vhfBands(solar, weather){
    ...
    const gust=n(c.wind_gusts_10m) ?? wind;
    const pressure=n(c.pressure_msl) ?? n(c.surface_pressure) ?? 1013;
    const kp=solar?.kp ?? 3;

    // Local VHF/UHF enhancement indicator.
    // This is deliberately an estimate, not a measured band opening.
    let tropo=0;
    if(humidity>=80) tropo+=2;
    else if(humidity>=65) tropo+=1;

    if(wind<=6) tropo+=2;
    else if(wind<=10) tropo+=1;

    if(gust<=12) tropo+=1;
    if(pressure>=1020) tropo+=1;

    const tropoGrade =
      tropo>=5 ? {label:"Enhanced",cls:"good"} :
      tropo>=3 ? {label:"Possible",cls:"fair"} :
                 {label:"Normal",cls:"neutral"};

    const aurora =
      kp>=6 ? {label:"Aurora possible",cls:"good"} :
      kp>=5 ? {label:"Aurora watch",cls:"fair"} :
              {label:"Aurora quiet",cls:"neutral"};

    const six =
      (solar?.sfi ?? 100)>=150 && kp<=3
        ? {label:"Check openings",cls:"fair"}
        : {label:"Normal / local",cls:"neutral"};

    return [
      {band:"6m", current:six, enhancement:"Sporadic-E / Tropo / Aurora — verify with live spots"},
